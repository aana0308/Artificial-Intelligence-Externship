...
if not (r"videos/example_04.mp4", False):
...
# otherwise, grab a reference to the video file
    ...
    vs = cv2.VideoCapture("videos/example_04.mp4")
...
while True:
    ...
    if "videos/example_04.mp4" is not None and frame is None:
        break;

    # resize the frame to have a maximum width of 500 pixels
    # (the less the data we have, the faster we can process it), then convert
    ...
    if "outputvideos/output_01.avi" is not None and writer is None:
        fourcc = cv2.VideoWriter_fourcc(*"MJPG")
        writer = cv2.VideoWriter(r"outputvideos/output_01.avi", fourcc, 30, (W, H), True)
    ...
    if totalFrames % skip_frames == 0:
        ...
        for i in np.arange(0, detections.shape[2]):
            ...
            if confidence > 0.4:
                ...
                # add the tracker to our list of trackers, so we can utilize it
                # during skip frames
